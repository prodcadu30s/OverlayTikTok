...
export function normalizeOverlay(input) {
  ...
  return {
    ...
    width,
    height,
    sourceWidth: Math.max(20, toInt(input.sourceWidth ?? input.frameWidth ?? width, width)),
    sourceHeight: Math.max(20, toInt(input.sourceHeight ?? input.frameHeight ?? height, height)),
    ...
    filter: ["none", "grayscale", "sepia", "blur", "contrast"].includes(input.filter) ? input.filter : "none",
    group: String(input.group || "").trim(),
    fit: ["fill", "contain", "cover"].includes(input.fit) ? input.fit : "fill",
